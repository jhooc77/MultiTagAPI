...
public class TagHandler_V1_19_R2 implements TagHandler {
    ...
    public TagHandler_V1_19_R2(MultiTagAPI plugin) {
        this.plugin = plugin;
                    ...
                    if (tag != null) {
                        ...
                        if (!tag.getEntityUUID().equals(player.getUniqueId())) {
                            ...
                            for (int i = 0; i < textLines.get(tag).size(); i++) {
                                ...
                                connection.send(new ClientboundSetEntityDataPacket(textLine.getId(), textLine.getEntityData().getNonDefaultValues()));

                                Snowball spliter = spliters.get(tag).get(i);
                                spliter.setPosRaw(x, y, z);
                                connection.send(new ClientboundAddEntityPacket(spliter));
                                connection.send(new ClientboundSetEntityDataPacket(spliter.getId(), spliter.getEntityData().getNonDefaultValues()));
    ...
    public void createTag(Tag tag) {
        ...
        for (int i = 0; i < tag.getTexts().size(); i++) {
            ...
            updateEntities.add(new ClientboundSetEntityDataPacket(line.getId(), line.getEntityData().getNonDefaultValues()));
            ...
            updateEntities.add(new ClientboundSetEntityDataPacket(spliter.getId(), spliter.getEntityData().getNonDefaultValues()));
    ...
    public void updateTag(Tag tag) {
        ...
        for(int i = 0; i < textLength; i++) {
                ...
                Snowball spliter = new Snowball(EntityType.SNOWBALL, ((CraftWorld) target.getWorld()).getHandle()) {
                    {
                        ...
                        spliters.add(this);
                        spawnEntities.add(new ClientboundAddEntityPacket(this));
                        updateEntities.add(new ClientboundSetEntityDataPacket(this.getId(), this.getEntityData().getNonDefaultValues()));
                    }
                };
                if (i == 0) {
                    FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
            ...
            updateEntities.add(new ClientboundSetEntityDataPacket(textLine.getId(), textLine.getEntityData().getNonDefaultValues()));
        }
